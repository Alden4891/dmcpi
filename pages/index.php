...
/*
	...
	//-------------------------------------------------------------------------------------------------------------
*/
  // ** LOAD ACTIVE ENCODING SESSION (AES) -----------------------------------------------------------------------
    $AES_ID = 0;
    $AES_PARTICULAR = "";
    $AES_DATE_START = "";
    $AES_DATE_END = "";
    ...
    $res_aes = mysqli_query($con, "SELECT ID, PARTICULAR,DATE_START, DATE_END, MONTHNO,`YEAR`, IF(DATE_END< NOW(),1,0) AS isLATE, CONCAT(MONTHNAME(DATE_END),' ',DAY(DATE_END),', ', YEAR(DATE_END)) AS DEADLINE FROM tbl_activities WHERE isactive=1;");
        

    if (mysqli_num_rows($res_aes) > 0) {
        $row_aes = mysqli_fetch_row($res_aes);
        $AES_ID         = $row_aes[0];
        $AES_PARTICULAR = $row_aes[1];
        $AES_DATE_START = $row_aes[2];
        $AES_DATE_END   = $row_aes[3];
        $AES_MONTHNO    = $row_aes[4];
        $AES_YEAR       = $row_aes[5];
        $AES_ISLATE     = $row_aes[6];
        $AES_DATELINE   = $row_aes[7];
    }

    $aes_notification = $AES_ISLATE==1?
      "<div class=\"alert alert-danger\" role=\"alert\">Encoding session for ".$arrMonths[$AES_MONTHNO]." $AES_YEAR has ended. You may continue to encode late receipt (subject for approval). </div>":
      "<div class=\"alert alert-info\" role=\"alert\">The deadline of encoding for ".$arrMonths[$AES_MONTHNO]." $AES_YEAR will be on $AES_DATELINE. </div>"
          ...
          <div class="left_col scroll-view">
            ...
            <div id="sidebar-menu" class="main_menu_side hidden-print main_menu">
              <div class="menu_section">
                ...
                <li class="active"><a href="?page=dashboard"><i class="fa fa-laptop"></i> Dashboard <span class="label label-success pull-right">1</span></a></li>
