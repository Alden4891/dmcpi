...
$aainsno = (isset($_REQUEST['aainsno'])?$_REQUEST['aainsno']:'');
...
if ($save_successful==1) {
   ...
   ");
	...
	for ($i=0; $i < $number_of_payment ; $i++) { 
		...
		$calc->setORDate($aaordate);
		$calc->setAmountPaid($aaamount_org);
		$calc->setLastInstallmentNo(0);
		$netAmount = $calc->compute();

		$c_AgentID = $AGENT_ID;
		$c_BranchID = $BM_ID;
		$c_Member_Code = $new_member_code;
		$c_PlanID = $PLAN_ID;
		$c_Amount_Paid = $aaamount_org;
		$c_NoOfPeriodPaid = $calc->getInstallmentNo();
		$c_AgentShareRate = $calc->getAGShareRate();
		$c_BMShareRate = $calc->getBMShareRate();
		$c_Mode_of_Computation = $calc->getModeOfComputation();
		$c_BMShareAmount = $calc->getBMShareAmount();
		$c_AgentShareAmount = $calc->getAGShareAmount();
		$c_Month = $calc->getMonthCovered();
		$c_Year = $calc->getYearCovered();
		$c_PeriodNo = $calc->getPeriodNo();
		$c_ORno = $aaornumber;
		$c_ORdate = $aaordate;
